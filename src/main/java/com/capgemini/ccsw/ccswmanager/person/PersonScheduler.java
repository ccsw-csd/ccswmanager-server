...
public class PersonScheduler {
	...
	public void scheduledTask() {
		...
		List<PersonEntity> filteredList = activePersons.stream().filter(two -> list.stream()
				  .anyMatch(one -> (checkUsernameOrSagaAreSame(two, one) && checkChangedParametersComparingPersonAndTperson(two, one, centerTranscodeEntityList))))
	...
	private boolean checkChangedParametersComparingPersonAndTperson(PersonEntity personEntity, TPersonEntity entityTmp, List<CenterTranscodeEntity> centerTranscodeEntityList) {
		boolean tmp = false;
		if (entityTmp != null) {
			...
			if(!entityTmp.getCenterTranscode().isEmpty() && !centerTranscodeEntityList.stream().filter(item -> item.getName().equals(entityTmp.getCenterTranscode())).findFirst().get().getCenter().getName().equals(personEntity.getCenter().getName())){
				personEntity.setCenter(centerTranscodeEntityList.stream().filter(item -> item.getName().equals(entityTmp.getCenterTranscode())).findFirst().get().getCenter());
				tmp = true;
			}
